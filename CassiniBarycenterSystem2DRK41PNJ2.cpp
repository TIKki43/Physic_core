//   g++ -std=c++20 -O2 -I . -I deps/cspice/include CassiniBarycenterSystem2DRK41PNJ2.cpp PreInitializer.cpp WorldPhysics.cpp Body.cpp Vec3.cpp Gravity.cpp Integrator.cpp Mat3.cpp deps/cspice/lib/libcspice.a -lraylib -lopengl32 -lgdi32 -lwinmm -static-libgcc -static-libstdc++  -o CassiniBarycenterSystem2DRK41PNJ2.exe
// RUN:
...
int main() {
    ...
    std::ofstream errorCsv("IntegrationErrors_Barycenter_RK41PN_Asteroids_900.csv"); // 3600, 900
    if (!errorCsv.is_open()) std::fprintf(stderr, "Cannot open IntegrationErrors_Barycenter_RK41PN_Asteroids_900.csv\n");
    ...
    InitWindow(WINDOW, WINDOW, "Cassini Barycenters RK4 + 1PN + Asteroids: WorldPhysics simulation(cyan) vs SPICE reference(orange)");
    SetTargetFPS(60);
    ...
    while (!WindowShouldClose()) {
        ...
        DrawText("Legend:  SIM = cyan   SPICE reference = orange   [RK4 + 1PN + Asteroids]", 10, y, 14, RAYWHITE); y += dy;
