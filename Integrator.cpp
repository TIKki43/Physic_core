...
void Integrator::RungeKutta4Integration1PN(std::vector<Body>& Bodies,
    ...
    auto ComputeAccelerations = [&](std::span<const Body> Bodies) {
        ...
        for (std::size_t a{}; a < size; ++a) {
            ...
            for (std::size_t b{}; b < size; ++b) {
                ...
                const Vec3 term2 =
                    (vA - vB) * ((muB / r2) * velocityProjection);

                const Vec3 term3 =
                    newtonAcc[b] * (3.5 * muB / r);
    ...
    for (int stage{1}; stage < 4; ++stage) {
        ...
        for (std::size_t i{}; i < size; ++i) {
            ...
            intermediateBody.SetPosition(
                fma(PositionDerivatives[previousOffset + i],
                    stepScale,
                    intermediateBody.GetPosition())
            );

            intermediateBody.SetVelocity(
                fma(VelocityDerivatives[previousOffset + i],
                    stepScale,
    ...
    for (std::size_t i{}; i < size; ++i) {
        const Vec3 PositionDelta =
            (PositionDerivatives[i]
            + PositionDerivatives[size + i] * 2.0
            + PositionDerivatives[2 * size + i] * 2.0
            + PositionDerivatives[3 * size + i]) * dt6;

        const Vec3 VelocityDelta =
            (VelocityDerivatives[i]
            + VelocityDerivatives[size + i] * 2.0
            + VelocityDerivatives[2 * size + i] * 2.0
            + VelocityDerivatives[3 * size + i]) * dt6;
