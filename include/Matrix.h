...
public:
  Matrix () = default;
  // Matrix(const T* inp, std::size_t size) : Data(new T[size]), Size(size) {
  // std::copy(inp, inp + size, Data); }
  Matrix (const T *inp, std::initializer_list<std::size_t> shape)
  { // inp = intializer list ToDo
    Rank = shape.size ();
  ...
  template <std::size_t N> Matrix (const T (&inp)[N])
  {
    Size = N;
    Rank = 1;
  ...
  Matrix (Matrix &&inp) noexcept
      : Data (inp.Data), Size (inp.Size), Shape (inp.Shape), Rank (inp.Rank)
  {
    inp.Data = nullptr;
    inp.Shape = nullptr;
    inp.Size = 0;
  ...
  T &
  operator[] (Indices... idxs)
  {
    if (sizeof...(Indices) != Rank)
      {
        Utils::panic ("Incorrect number of indices");
      }
    ...
    for (std::size_t idx{}; idx < Rank; ++idx)
      {
        if (indices[idx] >= Shape[idx])
          {
            Utils::panic ("Matrix index out of range");
          }
  ...
  void
  Reshape (const std::initializer_list<std::size_t> newShape)
  {
    ...
    if (newSize != Size)
      return Utils::panic ("Matrix sizes do not mach");
...
// 4 5 7   7 6 0
// 3 5 8   8 0 4
// 5 1 9,  5 2 2 Shapes = (rows, cols, batch, t) [2, 1] [1, 1, 1](Shapes = (4,
// 3, 2))  [0] Size = rows * cols * batch * ... * N * sizeof(T); el = A[rows -
// 2, cols - 1, batch - 0] 1 2 3 4 5 7 3 5 8 5 1 9 !!!!! 2 4 6 7 6 0 8 0 4 5 2
// 2
