...
void SetOrbit (Body &OrbitingBody, Body &CentralBody,
               const float SemiMajorAxis, const float Eccentricity,
               const float Inclination, const float AscendingLongitude,
               const float Periapsis, const float MeanAnomaly);
namespace Missions
{
namespace Cassini
{
// set 8 planet system barycenters and sun barycenter
void SetBarycenterSolarSystem (
    WorldPhysics &World, const std::string &FilePath, const std::string &Epoch,
    const std::string &Frame = "J2000", const std::string &Abcorr = "NONE",
