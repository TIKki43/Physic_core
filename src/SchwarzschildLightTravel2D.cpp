// g++ -std=c++20 -O2 -static-libgcc -static-libstdc++ -o
// SchwarzschildLightTravelDemo2D.exe SchwarzschildLightTravelDemo2D.cpp
// Ray.cpp RayTracer.cpp Body.cpp Vec3.cpp -lraylib -lopengl32 -lgdi32 -lwinmm
...
ScreenX (float simX)
{
  return WindowSize / 2.0f + simX * PixelsPerUnit;
}
float
ScreenY (float simY)
{
  return WindowSize / 2.0f - simY * PixelsPerUnit;
}
}
...
main ()
{
  ...
  Body BlackHole (Vec3 (0.0f, 0.0f, 0.0f), Vec3 (0.0f, 0.0f, 0.0f),
                  Demo::BlackHoleMass, Vec3 (), "BlackHole");

  // Photon starts far to the left, aimed toward the hole(+x). Ray ctor takes
  // Vec3&, so use named lvalues, not temporaries.
  ...
  Tracer.Rays[0].InitSchwarzschildMetricState (
      BlackHole, Demo::RayEnergy, Demo::RayAngularMomentum, 0.0f, Constants::c,
      Constants::G, Demo::RayDLambda);

  const float Rs
      = 2.0f
        * (Constants::G * Demo::BlackHoleMass / (Constants::c * Constants::c));
  ...
  InitWindow (Demo::WindowSize, Demo::WindowSize,
              "Schwarzschild Light Travel Demo 2D");
  SetTargetFPS (60);
  ...
  while (!WindowShouldClose ())
    {
      ...
      const Vector2 BhScreen{ Demo::ScreenX (BhPos.x),
                              Demo::ScreenY (BhPos.y) };

      // Photon sphere (1.5*Rs = 3*M_geo) as a faint reference ring
      DrawCircleLines ((int)BhScreen.x, (int)BhScreen.y,
                       1.5f * Rs * Demo::PixelsPerUnit, DARKGRAY);

      DrawCircleV (BhScreen, Rs * Demo::PixelsPerUnit, BLACK);
      DrawCircleLines ((int)BhScreen.x, (int)BhScreen.y,
                       Rs * Demo::PixelsPerUnit, ORANGE);

      // Ray trail.
      for (int i = 0; i < TrailCount; ++i)
        {
          const Vec3 &TrailPos = RayTrail[i];
          DrawCircleV (Vector2{ Demo::ScreenX (TrailPos.x),
                                Demo::ScreenY (TrailPos.y) },
                       1.5f, GREEN);
        }

      // Current photon position + direction indicator
      const Vector2 RayScreen{ Demo::ScreenX (RayNowPos.x),
                               Demo::ScreenY (RayNowPos.y) };
      DrawCircleV (RayScreen, 4.0f, RED);
      const Vec3 RayDirNow = Tracer.Rays[0].GetDirection ();
      DrawLineV (RayScreen,
                 Vector2{ RayScreen.x + RayDirNow.x * 25.0f,
                          RayScreen.y - RayDirNow.y * 25.0f },
                 YELLOW);

      DrawText (TextFormat ("r to hole: %.1f (Rs=%.1f)", RayR, Rs), 10, 10, 20,
                RAYWHITE);
      DrawText (TextFormat ("Status: %s",
                            Tracer.Rays[0].Active ? "TRAVELLING" : "CAPTURED"),
                10, 35, 20, RAYWHITE);
      DrawText (
          TextFormat ("Travel time: %.2f", Tracer.Rays[0].GetTravelTime ()),
          10, 60, 20, RAYWHITE);
