...
//   CassiniPhysicalSolarSystem2DRK4.cpp PreInitializer.cpp WorldPhysics.cpp
//   Body.cpp Vec3.cpp Gravity.cpp Integrator.cpp Mat3.cpp
//   deps/cspice/lib/libcspice.a -lraylib -lopengl32 -lgdi32 -lwinmm
//   -static-libgcc -static-libstdc++ -o CassiniPhysicalSolarSystem2DRK4.exe
//
//   g++ -std=c++20 -O2 -I . -I deps/cspice/include
//   CassiniPhysicalSolarSystem2DRK4.cpp PreInitializer.cpp WorldPhysics.cpp
//   Body.cpp Vec3.cpp Gravity.cpp Integrator.cpp Mat3.cpp
...
main ()
{
  ...
  // Focus targets = primaries that actually have moons (skip
  // Sun/Mercury/Venus).
  ...
  std::ofstream errorCsv (
      "IntegrationErrors_Physical_RK41PN_900.csv"); // physical bodies, RK4
  if (!errorCsv.is_open ())
    std::fprintf (stderr,
                  "Cannot open IntegrationErrors_Physical_RK4_900.csv\n");
  ...
  InitWindow (WINDOW, WINDOW,
              "Cassini Physical Solar System[RK4]: WorldPhysics "
  ...
  while (!WindowShouldClose ())
    {
      ...
      // VISUALIZATION ONLY: cycle the focused system (-1 -> EARTH -> MARS ->
      // ...
      // -> back)
      ...
      int y = 8;
      const int dy = 18;
      DrawText ("Legend:  SIM = cyan   SPICE reference = orange   [RK4]", 10,
                y, 14, RAYWHITE);
      y += dy;
      ...
      y += dy;
      DrawText (
          TextFormat ("Integrator  : RK4    Physics step: %.0f s   Speed: "
                      "%.2f sim-days/s",
      ...
      y += dy;
      DrawText (
          "F focus planet system (see moons) | SPACE pause | R restart | L "
          "lin/log | wheel zoom | drag pan | ESC",
          10, y, 12, LIGHTGRAY);
      y += dy;
