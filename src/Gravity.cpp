...
Vec3
Gravity::ComputeGravityForce (const Body &BodyA, const Body &BodyB) const
{
  ...
  double r = (BodyAPos - BodyBPos)
                 .LengthSquared (); // Squared distance between two bodies
...
Vec3
Gravity::ComputeJ2Acceleration (const Vec3 &DistanceDif, const double Mu,
                                const double J2, const double RefRadius,
                                const Vec3 &SpinAxis) const
{
  ...
  Vec3 aJ2_1 = DistanceDif * (5.0 * (rDotSpin) * (rDotSpin) / r2 - 1.0)
               - (SpinAxis * rDotSpin * 2.0);
