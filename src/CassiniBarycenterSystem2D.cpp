...
//
//   g++ -std=c++20 -O2 -I . -I deps/cspice/include
//   CassiniBarycenterSystemDemo2D.cpp PreInitializer.cpp WorldPhysics.cpp
//   Body.cpp Vec3.cpp Gravity.cpp Integrator.cpp Mat3.cpp
//   deps/cspice/lib/libcspice.a -lraylib -lopengl32 -lgdi32 -lwinmm
//   -static-libgcc -static-libstdc++  -o CassiniBarycenterSystemDemo2D.exe
// RUN:
...
static const double
    PHYSICS_STEP_SECONDS // 1 hour, conservative for semi-implicit Euler
    = 900.0;
static double simDaysPerSecond = 8.0; // simulation speed
static const double
    TRAIL_SAMPLE_SECONDS // sample trails once per simulated day
    = 86400.0;
static const int MAX_TRAIL_POINTS // cap stored points per body per trajectory
    = 400;
static const double MAX_ELAPSED_SECONDS // 5 years; loaded SPK coverage is
    = 1825.0 * 86400.0; // 2004-12-15..2010-01-10(~1835 days from t0)

static const int WINDOW = 1000;

static const double LIN_PX_PER_METER // log reference radius 1e9 m
    = 450.0 / 4.6e12;
           ...
           double &initialET)
{
  ...
  PreInitializer::Missions::Cassini::SetBarycenterSolarSystem (
      world, META_KERNEL, INITIAL_EPOCH);
...
main ()
{
  ...
  if (!errorCsv.is_open ())
    std::fprintf (stderr, "Cannot open IntegrationErrors.csv\n");
  ...
  InitWindow (WINDOW, WINDOW,
              "Cassini Barycenters: WorldPhysics simulation(cyan) vs SPICE "
              "reference(orange)");
  SetTargetFPS (60);
  ...
  while (!WindowShouldClose ())
    {
      ...
      if (IsKeyPressed (KEY_EQUAL) and IsKeyPressed (KEY_KP_ADD))
        simDaysPerSecond *= 1.5;
      if (IsKeyPressed (KEY_MINUS) and IsKeyPressed (KEY_KP_SUBTRACT))
        simDaysPerSecond /= 1.5;
          ...
          for (size_t k = 1; k < st.size (); ++k)
            {
              ...
              DrawLineV (worldToScreen (st[k - 1]), worldToScreen (st[k]),
                         Fade (SKYBLUE, 0.12f + 0.6f * a));
            }
          const auto &ot = views[i].spiceTrail;
          for (size_t k = 1; k < ot.size (); ++k)
            {
              float a = (float)k / (float)ot.size ();
              DrawLineV (worldToScreen (ot[k - 1]), worldToScreen (ot[k]),
                         Fade (ORANGE, 0.12f + 0.6f * a));
            }
        }
      ...
      for (int i = 0; i < n; ++i)
        {
          ...
          if (spiceNowOk[i])
            DrawCircleV (worldToScreen (spiceNow[i]), 4.0f, ORANGE);
      ...
      int y = 8;
      const int dy = 18;
      DrawText ("Legend:  SIM = cyan   SPICE reference = orange", 10, y, 14,
                RAYWHITE);
      y += dy;
      DrawText (TextFormat ("Start epoch : %s UTC", INITIAL_EPOCH), 10, y, 14,
                RAYWHITE);
      y += dy;
      ...
      DrawText (TextFormat ("SPICE epoch : %s  (ET %.1f)", utc, etNow), 10, y,
                14, RAYWHITE);
      y += dy;
      DrawText (TextFormat ("Physics step: %.0f s   Speed: %.2f sim-days/s",
      ...
      DrawText (TextFormat ("Scale: %s    Zoom: %.2f    %s",
                ...
                10, y, 14, paused ? YELLOW : RAYWHITE);
      y += dy;
      ...
      DrawText (saturnErrKm >= 0
                ...
                10, y, 14, ORANGE);
      y += dy;
      DrawText ("SPACE pause | R restart | L lin/log | +/- speed | wheel zoom "
                "| drag pan | ESC quit",
                10, y, 12, LIGHTGRAY);
      y += dy;
