...
void
WorldPhysics::SimulationStep (float DeltaTime)
{
  ...
  for (std::size_t i{}; i < Bodies.size (); ++i)
    {
      for (std::size_t j{ i + 1 }; j < Bodies.size (); ++j)
        {
          Vec3 ForceAB
              = GravityModel.ComputeGravityForce (Bodies[i], Bodies[j]);
          Forces[i] += ForceAB;
          Forces[j] -= ForceAB;
        }
    }
...
void
WorldPhysics::AddBody (const Body &Some)
{
  Bodies.push_back (Some);
}

void
WorldPhysics::AddBody (Body &&Some)
{
