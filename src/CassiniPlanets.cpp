//   g++ -std=c++17 -I deps/cspice/include CassiniPlanets.cpp
//   deps/cspice/lib/libcspice.a -o CassiniPlanets.exe
...
main ()
{
  ...
  std::printf (
      "Epoch    : 2005-01-01T00:00:00 UTC  (ET = %.6f s past J2000)\n", et);
  ...
  std::printf (
      "Targets are planet CENTERS(e.g. JUPITER = 599, not barycenter 5).\n\n");
      ...
      if (failed_c ())
      ...
      else
        {
          std::printf ("%-18s : X = %20.3f   Y = %20.3f   Z = %20.3f  [km]\n",
                       planets[i].bary, planets[i].state[0],
                       planets[i].state[1], planets[i].state[2]);
        }
  ...
  for (int i = 0; i < 9; i++)
    {
      ...
      Body Object (velocity, position, MU_CI / Constants::G, Vec3 (),
                   planets[i].name);
    }
  ...
  // for (int i = 0; i < 8; ++i) {
  //     SpiceDouble lt = 0.0;
  //     spkpos_c(planets[i].name, et, frame, abcorr, observer, planets[i].pos,
  //     &lt);
  ...
  //         std::printf(" -> %s\n", shortmsg);
  //         std::printf("           Not substituting the barycenter
  //         silently.\n");
  //     } else {
