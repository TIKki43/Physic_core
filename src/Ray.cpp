...
float
Ray::GetFrequency () const
{
  return WaveSpeed / Wavelength;
}

void
Ray::SetPosition (const Vec3 &NewPosition)
{
  Position = NewPosition;
}
Vec3
Ray::GetPosition () const
{
  return Position;
}

void
Ray::SetDirection (const Vec3 &NewDirection)
{
...
float
Ray::GetTravelDistance () const
{
  return TravelDistance;
}
float
Ray::GetTravelTime () const
{
  return TravelTime;
}

float
Ray::AdvanceByTime (const float DeltaTime)
{
...
float
Ray::AdvanceByDistance (const float DeltaDistance)
{
...
void
Ray::InitSchwarzschildMetricState (Body &MassiveBody, float InitialE,
                                   float InitialL, float Initialt,
                                   float Initialc, float InitialG,
...
void
Ray::SchwarzschildMetric (Body &MassiveBody)
{
  f = 1.0f - 2.0f * M_geo / (r);
  ...
  Vec3 NewRel (r * std::cos (phi), r * std::sin (phi), 0.0f);

  this->SetPosition (MassiveBody.GetPosition () + NewRel);
}

void
Ray::SchwarzschildStep (Body &MassiveBody)
{
