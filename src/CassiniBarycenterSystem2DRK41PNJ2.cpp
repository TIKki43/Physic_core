...
//   CassiniBarycenterSystem2DRK41PNJ2.cpp PreInitializer.cpp WorldPhysics.cpp
//   Body.cpp Vec3.cpp Gravity.cpp Integrator.cpp Mat3.cpp
//   deps/cspice/lib/libcspice.a -lraylib -lopengl32 -lgdi32 -lwinmm
//   -static-libgcc -static-libstdc++  -o CassiniBarycenterSystem2DRK41PNJ2.exe
// RUN:
//   ./CassiniBarycenterSystem2DRK41PNJ2.cpp
...
static const double PHYSICS_STEP_SECONDS = 900.0; // RK4 physics step
static double simDaysPerSecond = 8.0;             // simulation speed
static const double TRAIL_SAMPLE_SECONDS
    = 86400.0; // sample trails once per simulated day
static const int MAX_TRAIL_POINTS
    = 400; // cap stored points per body per trajectory
static const double MAX_ELAPSED_SECONDS
    = 1825.0 * 86400.0; // 5 years; loaded SPK coverage is
                        // 2004-12-15..2010-01-10(~1835 days from t0)
...
main ()
{
  ...
  std::ofstream errorCsv (
      "IntegrationErrors_Barycenter_RK41PN_Asteroids_900.csv"); // 3600, 900
  if (!errorCsv.is_open ())
    std::fprintf (
        stderr,
        "Cannot open IntegrationErrors_Barycenter_RK41PN_Asteroids_900.csv\n");
  ...
  InitWindow (WINDOW, WINDOW,
              "Cassini Barycenters RK4 + 1PN + Asteroids: WorldPhysics "
  ...
  while (!WindowShouldClose ())
    {
      ...
      if (!paused && coverageError.empty ())
        {
          ...
          while (accumulator >= PHYSICS_STEP_SECONDS
                 ...
                 && guard < 5000)
            {
              world.Euler.RungeKutta4Integration1PN (
      ...
      const int dy = 18;
      DrawText (
          "Legend:  SIM = cyan   SPICE reference = orange   [RK4 + 1PN + "
          "Asteroids]",
          10, y, 14, RAYWHITE);
      y += dy;
