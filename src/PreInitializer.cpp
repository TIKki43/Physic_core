...
void
PreInitializer::SetOrbit (Body &OrbitingBody, Body &CentralBody, const float a,
                          const float e, const float i, const float w,
                          const float om, const float M)
{
  ...
  vy_2d = a * std::sqrt (1 - e * e) * std::cos (E)
          * (n / (1 - e * std::cos (E)));
  ...
  Mat3 GeneralRotation
      = Mat3::RotationZ (om) * Mat3::RotationX (i) * Mat3::RotationZ (w);

  OrbitingBody.SetPosition (CentralBody.GetPosition ()
                            + GeneralRotation * pos_3d);
  OrbitingBody.SetVelocity (CentralBody.GetVelocity ()
                            + GeneralRotation * v_3d);
};

void
PreInitializer::Missions::Cassini::SetBarycenterSolarSystem (
    WorldPhysics &World, const std::string &FilePath // std::filesystem::path
    ,
  ...
  if (failed_c ())
    {
      ...
      Utils::panic ("Can't read meta-kernel!");
    }

  // Epoch: 2005-01-01T00:00:00 UTC  -> ephemeris time(ET, seconds past J2000
  // TDB)
  SpiceDouble et = 0.0;
  ...
  if (failed_c ())
    {
      ...
      Utils::panic ("Can't find epoch in time intervals!");
    }
  ...
  for (std::size_t i{}; i < 9; ++i)
    {
      SpiceDouble lt = 0.0;
      SpiceInt dimension;
      spkezr_c (planets[i].bary, et, frame, abcorr, observer, planets[i].state,
                &lt);
      if (failed_c ())
        {
          ...
          std::printf ("%-18s : unavailable -> %s\n", planets[i].bary,
                       shortmsg);
          kclear_c ();
          Utils::panic ("No enough data to spkezr_c!");
        }

      std::printf ("%-18s : X = %20.3f   Y = %20.3f   Z = %20.3f  [km]\n",
                   planets[i].bary, planets[i].state[0], planets[i].state[1],
                   planets[i].state[2]);
      ...
      if (failed_c ())
        {
          ...
          std::printf ("%-18s : GM unavailable -> %s\n", planets[i].bary,
                       message);
          kclear_c ();
          Utils::panic ("No enough data to bodvrd_c!");
        }
    }

  for (std::size_t i{}; i < 9; i++)
    {
      Vec3 position (planets[i].state[0] * 1000.0,
                     planets[i].state[1] * 1000.0,
                     planets[i].state[2] * 1000.0); // m
      Vec3 velocity (planets[i].state[3] * 1000.0,
                     planets[i].state[4] * 1000.0,
                     planets[i].state[5] * 1000.0); // m / s
      double MU_SI = planets[i].GM * 1e9;           // m^3 * s^-2
      Body Object (velocity, position, MU_SI / Constants::G, Vec3 (), 0.0, 0.0,
                   Vec3 (), planets[i].bary);
...
PreInitializer::Missions::Cassini::SetPhysicalSolarSystem (
    WorldPhysics &World, const std::string &FilePath, const std::string &Epoch,
    const std::string &Frame, const std::string &Abcorr,
    const std::string &Observer)

{
  ...
  std::array EarthSystem{
    SpaceObject{ .name = "EARTH",
                 .J2 = Constants::AstroDynamicConstants::EarthJ2,
                 .RefRadius
                 = Constants::AstroDynamicConstants::EarthRefRadius },
    SpaceObject{ "MOON" }
  };
  std::array MarsSystem{
    SpaceObject{ .name = "MARS",
                 .J2 = Constants::AstroDynamicConstants::MarsJ2,
                 .RefRadius
                 = Constants::AstroDynamicConstants::MarsRefRadius },
    SpaceObject{ "PHOBOS" }, SpaceObject{ "DEIMOS" }
  };

  std::array JupiterSystem{
    SpaceObject{ .name = "JUPITER",
                 .J2 = Constants::AstroDynamicConstants::JupiterJ2,
                 .RefRadius
                 = Constants::AstroDynamicConstants::JupiterRefRadius },
    SpaceObject{ "IO" },
    SpaceObject{ "EUROPA" },
    SpaceObject{ "GANYMEDE" },
    SpaceObject{ "CALLISTO" },
    SpaceObject{ "AMALTHEA" },
    SpaceObject{ "THEBE" },
    SpaceObject{ "ADRASTEA" },
    SpaceObject{ "METIS" }
  };

  std::array SaturnSystem{
    SpaceObject{ .name = "SATURN",
                 .J2 = Constants::AstroDynamicConstants::SaturnJ2,
                 .RefRadius
                 = Constants::AstroDynamicConstants::SaturnRefRadius },
    SpaceObject{ "MIMAS" },
    SpaceObject{ "ENCELADUS" },
    SpaceObject{ "TETHYS" },
    SpaceObject{ "DIONE" },
    SpaceObject{ "RHEA" },
    SpaceObject{ "TITAN" },
    SpaceObject{ "HYPERION" },
    SpaceObject{ "IAPETUS" },
    SpaceObject{ "PHOEBE" },
    SpaceObject{ "JANUS" },
    SpaceObject{ "EPIMETHEUS" },
    SpaceObject{ "HELENE" },
    SpaceObject{ "ATLAS" },
    SpaceObject{ "PROMETHEUS" },
    SpaceObject{ "PANDORA" }
  };

  std::array UranusSystem{
    SpaceObject{ .name = "URANUS",
                 .J2 = Constants::AstroDynamicConstants::UranusJ2,
                 .RefRadius
                 = Constants::AstroDynamicConstants::UranusRefRadius },
    SpaceObject{ "ARIEL" },
    SpaceObject{ "UMBRIEL" },
    SpaceObject{ "TITANIA" },
    SpaceObject{ "OBERON" },
  ...
  std::array NeptuneSystem{
    SpaceObject{ .name = "NEPTUNE",
                 .J2 = Constants::AstroDynamicConstants::NeptuneJ2,
                 .RefRadius
                 = Constants::AstroDynamicConstants::NeptuneRefRadius },
    SpaceObject{ "TRITON" }
  };
  ...
  std::array<std::span<SpaceObject>, 10> Systems{
    std::span{ SunSystem },     std::span{ MercurySystem },
    std::span{ VenusSystem },   std::span{ EarthSystem },
    std::span{ MarsSystem },    std::span{ JupiterSystem },
    std::span{ SaturnSystem },  std::span{ UranusSystem },
    std::span{ NeptuneSystem }, std::span{ PlutoSystem },
  };
      ...
      for (SpaceObject &obj : system)
        {
          ...
          Vec3 position (obj.state[0] * 1000.0, obj.state[1] * 1000.0,
                         obj.state[2] * 1000.0); // m
          Vec3 velocity (obj.state[3] * 1000.0, obj.state[4] * 1000.0,
                         obj.state[5] * 1000.0); // m / s
          ...
          if (obj.J2 != 0.0)
            {
              ...
              SpiceBoolean found{};
              // getting name
              cnmfrm_c (obj.name, sizeof (bodyFrame), &frameCode, bodyFrame,
                        &found);
              if (!found)
                {
                  std::printf ("%-18s : body-fixed frame not found\n",
                               obj.name);
                  kclear_c ();
                  Utils::panic ("Can't find body-fixed frame!");
                }
              ...
              if (failed_c ())
                {
                  ...
                  kclear_c ();
                  Utils::panic ("Can't transform spin axis!");
                }
              ...
              SpinAxis = Vec3{ SpinAxisFrame[0], SpinAxisFrame[1],
                               SpinAxisFrame[2] };

              SpinAxis.Normalize ();
            }

          Body Object (velocity, position, MU_SI / Constants::G, Vec3 (),
                       obj.J2, obj.RefRadius, SpinAxis, obj.name);
          World.AddBody (Object);
        }
    }
}
  ...
  for (std::size_t i{}; i < Asteroids.size (); ++i)
    {
      SpiceDouble lt{};
      SpiceInt dimension{};
      spkezr_c (Asteroids[i].ID, et, frame, abcorr, observer,
                Asteroids[i].State, &lt);
      if (failed_c ())
        {
          SpiceChar shortmsg[26];
          getmsg_c ("SHORT", sizeof (shortmsg), shortmsg);
      ...
      std::printf ("%-18s : X = %20.3f   Y = %20.3f   Z = %20.3f  [km]\n",
                   Asteroids[i].Name, Asteroids[i].State[0],
                   Asteroids[i].State[1], Asteroids[i].State[2]);
      ...
      if (failed_c ())
        {
          ...
          std::printf ("%-18s : GM unavailable -> %s\n", Asteroids[i].Name,
                       message);
          kclear_c ();
          Utils::panic ("No enough data to bodvcd_c!");
        }
    }

  for (std::size_t i{}; i < Asteroids.size (); i++)
    {
      Vec3 position (Asteroids[i].State[0] * 1000.0,
                     Asteroids[i].State[1] * 1000.0,
                     Asteroids[i].State[2] * 1000.0); // m
      Vec3 velocity (Asteroids[i].State[3] * 1000.0,
                     Asteroids[i].State[4] * 1000.0,
                     Asteroids[i].State[5] * 1000.0); // m / s
